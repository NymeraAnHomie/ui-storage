...
function library:tween(obj, properties)
	...
	local tween =
		tween_service:Create(obj, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), properties)
	tween:Play()
	return tween
end
...
function library:window(properties)
	...
	task.spawn(function()
		while task.wait(0.6) do
			watermark.change_text(string.format("Suprahook - v1.13b - DEV - %s", os.date("%b %d %Y - %H:%M:%S")))
		end
	end)
